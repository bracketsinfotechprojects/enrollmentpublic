<?php
/**
 * generate_installment_pdf_file()
 * Generates a TCPDF invoice for the given installment and saves it to
 * uploads/invoices/. Returns ['success'=>bool, 'file_path'=>..., 'rel_path'=>...].
 */
function generate_installment_pdf_file(int $inst_id, mysqli $connection): array
{
    // ── Fetch data ────────────────────────────────────────────────────────────
    ...
    // ── Suppress default TCPDF header/footer ─────────────────────────────────
    if (!class_exists('NCA_PDF')) {
    ...
    // ── Helper ────────────────────────────────────────────────────────────────
    $hsc = function($v) { return htmlspecialchars_decode(htmlspecialchars((string)($v ?? ''), ENT_QUOTES)); };
    ...
    $pdf->SetFont('helvetica', '', 7);
    $pdf->SetTextColor(155, 180, 215);
    $sid_str = ($d['stu_id'] ?? '') . ($d['email_address'] ? ($d['stu_id'] ? '  ·  ' : '') . $d['email_address'] : '');
    ...
    foreach ($cw as $k => $w) {
        $pdf->SetXY($x, $tY);
        $pdf->Cell($w, 7, $ch[$k], 'B', 0, $ca[$k], true);
        $x += $w;
    }
    $pdf->Ln(7);
    ...
    $x = 15;
    $pdf->SetXY($x, $yRow); $pdf->Cell($cw[0], $row_h, '1', 'B', 0, 'C', true); $x += $cw[0];
    $pdf->SetXY($x, $yRow); $pdf->MultiCell($cw[1], $row_h, $desc, 'B', 'L', true, 0, '', '', true, 0, false, true, $row_h, 'M'); $x += $cw[1];
    $pdf->SetXY($x, $yRow); $pdf->Cell($cw[2], $row_h, '1', 'B', 0, 'C', true); $x += $cw[2];
    $pdf->SetXY($x, $yRow); $pdf->Cell($cw[3], $row_h, '$' . number_format($amount, 2), 'B', 0, 'R', true); $x += $cw[3];
    $pdf->SetXY($x, $yRow); $pdf->Cell($cw[4], $row_h, $gst > 0 ? '$' . number_format($gst, 2) : chr(226).chr(128).chr(148), 'B', 0, 'R', true); $x += $cw[4];
    $pdf->SetFont('helvetica', 'B', 8.5);
    $pdf->SetXY($x, $yRow); $pdf->Cell($cw[5], $row_h, '$' . number_format($total, 2), 'B', 0, 'R', true);
    $pdf->Ln($row_h);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //  PAID watermark + payment details
    // ─────────────────────────────────────────────────────────────────────────
    if ($is_paid) {
        ...
        if (!empty($d['payment_method'])) {
            ...
            if (!empty($d['receiver_name'])) $pdetail .= '  ·  Received by: ' . $d['receiver_name'];
    ...
    $safe_inv = preg_replace('/[^A-Za-z0-9\-]/', '', $d['invoice_number'] ?? $inst_id);
    $filename  = 'NCA-Invoice-' . $safe_inv . '-inst' . $inst_id . '.pdf';
    $file_path = $upload_dir . $filename;
    $rel_path  = 'uploads/invoices/' . $filename;
